...
class AcquisitionLP(AcquisitionBase):
    ...
    :param model: model of the class GauOptX
    :param space: design space of the class GauOptX.
    :param optimizer: optimizer of the class GauOptX.
    :param acquisition: acquisition function of the class GauOptX
    ...
    def update_batches(self, X_batch, L, Min):
        """
        Updates the batches internally and pre-computes the parameters for penalization.
        """
    ...
    def _hammer_function(self, x, x0, r_x0, s_x0):
        ...
        """
        return norm.logcdf((
            np.sqrt((np.square(np.atleast_2d(x)[:, None, :] - np.atleast_2d(x0)[None, :, :])).sum(-1)) - r_x0
        ) / s_x0)
