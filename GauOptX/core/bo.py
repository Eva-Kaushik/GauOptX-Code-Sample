...
class BO(object):
    ...
    :param de_duplication: GauOptX DuplicateManager class. Avoids re-evaluating the objective at previous, pending or infeasible locations (default, False).
    """
    ...
    def run_optimization(self, max_iter = 0, max_time = np.inf,  eps = 1e-8, context = None, verbosity=False, save_models_parameters= True, report_file = None, evaluations_file = None, models_file=None):
        ...
        if self.save_models_parameters == True:
            if not (isinstance(self.model, GauOptX.models.GPModel) or isinstance(self.model, GauOptX.models.GPModel_MCMC)):
    ...
    def _print_convergence(self):
        ...
        if self.verbosity:
            ...
            if self.initial_iter:
                print('** GauOptX Bayesian Optimization class initialized successfully **')
    ...
    def save_report(self, report_file= None):
        ...
        with open(report_file,'w') as file:
            ...
            file.write('-----------------------------' + ' GauOptX Report file ' + '-----------------------------------\n')
            file.write('GauOptX Version ' + str(GauOptX.__version__) + '\n')
